...
- macOS
  - macOS 12.0+ (Universal binary supporting both Apple Silicon and Intel)
  - for JPEG compression: install libjpeg-turbo via Homebrew (`brew install libjpeg-turbo`)
  - all other compression options and raw transmission work without extra dependencies
...
For automatic installation just run the provided `.pkg` installer, then restart OBS.

For manual installation extract the downloaded `.tar.xz` file (= copy the contained `xObsBeam.plugin` bundle) into:
...
**Note on JPEG compression:** The JPEG compression option requires libjpeg-turbo to be installed on your system. Install it via Homebrew:

```
brew install libjpeg-turbo
```

All other compression options (QOI, QOY, QOIR, Density, LZ4) and raw transmission work without any extra dependencies.
...
**Named pipe connections work out of the box.** xObsBeam uses Unix domain sockets for named pipes, which .NET locates via the `TMPDIR` environment variable. Because Parall overrides `HOME` per instance (which would otherwise isolate any home-relative path), xObsBeam forces the pipe socket directory to `/tmp` on macOS — a shared location that is not redirected by Parall, so both the sender and receiver instances can find each other's pipes.

If you need a custom shared directory instead of `/tmp` (e.g. for a multi-user system where `/tmp` is not desirable), set the `XOBSBEAM_PIPE_DIR` environment variable to the same absolute path on **both** OBS instances (e.g. via Parall's custom environment variables feature, or by launching OBS from a shell that exports it):

```
XOBSBEAM_PIPE_DIR=/Users/Shared/xobsbeam-pipes
```

Make sure the directory exists and is writable by both instances. The socket path (directory + `CoreFxPipe_<identifier>`) must stay under 104 characters, so keep the identifier and directory path short.

If you encounter any issues with named pipes in a multi-instance setup, using a **TCP socket connection to `127.0.0.1`** is a reliable alternative that does not depend on `TMPDIR` or shared directories.
