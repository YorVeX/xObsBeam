...
In order to use the wrapper class the binary QoirLib library is needed. For Windows the necessary `QoirLib.dll` file and for Linux the `libQoirLib.so` file compiled on Ubuntu 22.04 (glibc 2.35) are directly provided in the [binaries](binaries) folder for your convenience. On macOS the `libQoirLib.dylib` is built from source as part of the xObsBeam macOS build scripts and bundled inside the `.plugin` bundle. Simply copy them to the same folder where your xObsBeam plugin file is located, other folders within your system PATH should also work.

If you want to build it yourself just compile the QoirLib.h file using the Visual Studio 2022 project provided here for Windows, or run `make` on Linux or macOS. The makefile detects the platform automatically and produces `libQoirLib.so` on Linux and `libQoirLib.dylib` on macOS. To build a single architecture on macOS (for the universal build) pass the `ARCH` variable, e.g. `make ARCH=arm64` or `make ARCH=x86_64`.
